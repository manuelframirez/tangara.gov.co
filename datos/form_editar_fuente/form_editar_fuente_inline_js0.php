...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_editar_fuente_inline']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
<?php
  ...
  {
?>  
  if (parent && parent.nmsc_iframe_liga_form_editar_fuente.scInlineFormReceive)
  {
    parent.nmsc_iframe_liga_form_editar_fuente.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_editar_fuente_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
?>  
  ...
  {
    parent.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_editar_fuente_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit = new NM_critica(2);
 NM_tab_crit[1] = new NM_critica('F1', 'texto_fuente_', 'Texto Fuente', 'naomask', 'lista', 'cxab', 32767, 'TUDO');
 NM_tab_crit[2] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(2);
 NM_tab_crit_1[1] = new NM_critica('F1', 'texto_fuente_', 'Texto Fuente', 'naomask', 'lista', 'cxab', 32767, 'TUDO');
 NM_tab_crit_1[2] = new NM_critica('NM_final');
